...
CREATE TABLE IF NOT EXISTS `users` (
    ...
    `status` VARCHAR(20) DEFAULT 'Active' COMMENT '账号激活/封禁状态',
...
CREATE TABLE IF NOT EXISTS `teams` (
    ...
    `mode` VARCHAR(50) COMMENT '授课或执行模式 (CC, remote RE等)',
    `poc_email` VARCHAR(100) COMMENT 'POC 电子邮件',
    `team_size` INT COMMENT '球队规模',
    `potential` VARCHAR(255) COMMENT '潜力',
    `current_issue` TEXT COMMENT '当前问题',
    `instructor_one` VARCHAR(100) COMMENT '教官-1',
    `issue_priority` VARCHAR(50) COMMENT '议题优先级',
    `team_meeting` VARCHAR(255) COMMENT '团队会议时间',
    `project_acceptance` VARCHAR(255) COMMENT '项目接受状态',
    `team_activity` TEXT COMMENT '团队活动',
    `sponsor_contacted` BOOLEAN DEFAULT FALSE COMMENT '赞助商联系情况',
    `met_sponsor` BOOLEAN DEFAULT FALSE COMMENT '是否见过赞助商',
    `product_confidence` VARCHAR(100) COMMENT '产品信心级别',
    `need_help_with_prototype` BOOLEAN DEFAULT FALSE COMMENT '需要原型开发帮助',
    `team_message` TEXT COMMENT '团队公开信息或备注',
    `team_status` VARCHAR(255) COMMENT '球队状态',
    `project_assigned_id` BIGINT COMMENT '分配的项目提案ID',
    `average_wan` DECIMAL(5,2) COMMENT '队伍整体平均成绩分',
    `course_id` BIGINT COMMENT '所属课程ID',
    `point_of_contact_id` BIGINT COMMENT '队伍代表联络人',
...
CREATE TABLE IF NOT EXISTS `student_profiles` (
    ...
    `degree_type` VARCHAR(50) COMMENT '学位类型 (DegreeType)',
    `contact_email` VARCHAR(100) COMMENT '首选联系邮件 (如果为空则向users.email发信)',
    `course_id` BIGINT COMMENT '学生所在课程ID',
